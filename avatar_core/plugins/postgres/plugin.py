...
from avatar_core.models import Item
from avatar_core.plugins.base.plugin import Plugin
from avatar_core.plugins.postgres.settings import PostgresSettings
...
class PostgresPlugin(Plugin):
    ...
    async def unique_id(self, item_type: str, item_id: str | None = None) -> str:
        async with self.session() as session:
            item = Item(item_id=item_id, item_type=item_type)
            session.add(item)
    ...
    async def depends_unique_id(self) -> Callable[[str | None, str], str]:
